...
export default async function PatientLayout({ children }) {
  ...
  if (!valid || decoded.role !== 'patient') {
    redirect('/login/patient');
  }

  const menuItems = [
    { href: '/patient/dashboard', label: 'Dashboard', icon: LayoutDashboard },
    { href: '/patient/my-appointments', label: 'My Appointments', icon: Calendar },
        ...
        <nav className="flex-1 p-4 space-y-1">
          {menuItems.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="flex items-center gap-3 px-4 py-3 rounded-lg text-slate-600 dark:text-slate-400 hover:bg-blue-50 dark:hover:bg-blue-500/10 hover:text-blue-600 dark:hover:text-blue-400 transition-all"
            >
              <item.icon size={20} />
