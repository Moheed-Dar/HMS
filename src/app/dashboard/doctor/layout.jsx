...
export default async function DoctorLayout({ children }) {
  ...
  if (!valid || decoded.role !== 'doctor') {
    redirect('/login/doctor');
  }

  const menuItems = [
    { href: '/doctor/dashboard', label: 'Dashboard', icon: LayoutDashboard },
    { href: '/doctor/my-patients', label: 'My Patients', icon: Users },
        ...
        <nav className="flex-1 p-4 space-y-1">
          {menuItems.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="flex items-center gap-3 px-4 py-3 rounded-lg text-slate-600 dark:text-slate-400 hover:bg-green-50 dark:hover:bg-green-500/10 hover:text-green-600 dark:hover:text-green-400 transition-all"
            >
              <item.icon size={20} />
